import { NgModule } from '@angular/core';
import { TestBed } from '@angular/core/testing';
import { EffectsModule } from '@ngrx/effects';
import { StoreModule, Store } from '@ngrx/store';
import { NxModule } from '@nrwl/angular';
import { readFirst } from '@nrwl/angular/testing';

import * as TransfersActions from './transfers.actions';
import { TransfersEffects } from './transfers.effects';
import { TransfersFacade } from './transfers.facade';
import { TransfersEntity } from './transfers.models';
import {
  TRANSFERS_FEATURE_KEY,
  State,
  initialState,
  reducer,
} from './transfers.reducer';
import * as TransfersSelectors from './transfers.selectors';

interface TestSchema {
  transfers: State;
}

describe('TransfersFacade', () => {
  let facade: TransfersFacade;
  let store: Store<TestSchema>;
  const createTransfersEntity = (id: string, name = ''): TransfersEntity => ({
    id,
    name: name || `name-${id}`,
  });

  describe('used in NgModule', () => {
    beforeEach(() => {
      @NgModule({
        imports: [
          StoreModule.forFeature(TRANSFERS_FEATURE_KEY, reducer),
          EffectsModule.forFeature([TransfersEffects]),
        ],
        providers: [TransfersFacade],
      })
      class CustomFeatureModule {}

      @NgModule({
        imports: [
          NxModule.forRoot(),
          StoreModule.forRoot({}),
          EffectsModule.forRoot([]),
          CustomFeatureModule,
        ],
      })
      class RootModule {}
      TestBed.configureTestingModule({ imports: [RootModule] });

      store = TestBed.inject(Store);
      facade = TestBed.inject(TransfersFacade);
    });

    /**
     * The initially generated facade::loadAll() returns empty array
     */
    it('loadAll() should return empty list with loaded == true', async () => {
      let list = await readFirst(facade.allTransfers$);
      let isLoaded = await readFirst(facade.loaded$);

      expect(list.length).toBe(0);
      expect(isLoaded).toBe(false);

      facade.init();

      list = await readFirst(facade.allTransfers$);
      isLoaded = await readFirst(facade.loaded$);

      expect(list.length).toBe(0);
      expect(isLoaded).toBe(true);
    });

    /**
     * Use `loadTransfersSuccess` to manually update list
     */
    it('allTransfers$ should return the loaded list; and loaded flag == true', async () => {
      let list = await readFirst(facade.allTransfers$);
      let isLoaded = await readFirst(facade.loaded$);

      expect(list.length).toBe(0);
      expect(isLoaded).toBe(false);

      store.dispatch(
        TransfersActions.loadTransfersSuccess({
          transfers: [
            createTransfersEntity('AAA'),
            createTransfersEntity('BBB'),
          ],
        })
      );

      list = await readFirst(facade.allTransfers$);
      isLoaded = await readFirst(facade.loaded$);

      expect(list.length).toBe(2);
      expect(isLoaded).toBe(true);
    });
  });
});
